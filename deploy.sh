...
calc_code_hash() {
    {
        ...
        cat Cargo.toml Cargo.lock entrypoint.sh gunicorn_conf.py 2>/dev/null
        ...
        find apps -type f \( -name "*.rs" -o -name "Cargo.toml" -o -name "README.md" \) 2>/dev/null | sort | xargs cat 2>/dev/null
        find crates -type f \( -name "*.rs" -o -name "Cargo.toml" \) 2>/dev/null | sort | xargs cat 2>/dev/null
    } | md5sum | cut -d' ' -f1
}
...
# 构建应用镜像
build_app() {
    echo ">>> Building app image (python + rust gateway)..."
    generate_version_file
