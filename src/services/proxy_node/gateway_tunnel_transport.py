"""
Gateway tunnel relay transport。

Worker 通过本机 gateway relay 访问 tunnel 数据面，不再维护 /worker WebSocket。
"""
...
from .tunnel_config import get_tunnel_relay_config
...
class GatewayTunnelTransport(httpx.AsyncBaseTransport):
    """通过本机 gateway relay 转发请求的 httpx transport。"""

    def __init__(self, node_id: str, timeout: float = 60.0) -> None:
        self._node_id = node_id
        self._timeout = timeout

        config = get_tunnel_relay_config()
    ...
    async def handle_async_request(self, request: httpx.Request) -> httpx.Response:
        ...
        if not config.enabled:
            raise httpx.ConnectError("gateway tunnel relay is unavailable outside docker runtime")
        ...
        try:
            ...
            raise httpx.ConnectError(f"gateway tunnel relay connect failed: {exc}") from exc
        except httpx.TimeoutException as exc:
            raise httpx.ConnectError(f"gateway tunnel relay timeout: {exc}") from exc
        ...
        if tunnel_error:
            ...
            if tunnel_error == "timeout":
                raise httpx.ReadTimeout(message or "tunnel relay timed out")
            raise httpx.ConnectError(message or f"gateway tunnel relay error: {tunnel_error}")

        return httpx.Response(
            status_code=relay_response.status_code,
            headers=httpx.Headers(relay_response.headers),
            stream=TunnelRelayResponseStream(relay_response),
            request=request,
        )
...
class TunnelRelayResponseStream(httpx.AsyncByteStream):
