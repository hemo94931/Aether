...
const ADMIN_PROXY_NODES_RUST_BACKEND_DETAIL: &str =
    "Admin proxy nodes routes require Rust maintenance backend";

fn build_admin_proxy_nodes_maintenance_response() -> Response<Body> {
    (
        http::StatusCode::SERVICE_UNAVAILABLE,
        Json(json!({ "detail": ADMIN_PROXY_NODES_RUST_BACKEND_DETAIL })),
    )
        .into_response()
}
...
pub(crate) async fn maybe_build_local_admin_proxy_nodes_response(
        ...
        )
    {
        if !state.data.has_proxy_node_reader() {
            return Ok(Some(build_admin_proxy_nodes_maintenance_response()));
        }
    ...
    Ok(Some(build_admin_proxy_nodes_maintenance_response()))
}
