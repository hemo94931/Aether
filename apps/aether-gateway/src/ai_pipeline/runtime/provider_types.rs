...
pub(crate) struct ProviderOAuthTemplate {
    ...
    pub(crate) client_secret: &'static str,
...
pub(crate) fn provider_type_supports_model_fetch(provider_type: &str) -> bool {
    !matches!(
        provider_type.trim().to_ascii_lowercase().as_str(),
...
pub(crate) fn provider_type_supports_local_openai_chat_transport(provider_type: &str) -> bool {
    !matches!(
        provider_type.trim().to_ascii_lowercase().as_str(),
...
pub(crate) fn provider_type_supports_local_same_format_transport(provider_type: &str) -> bool {
    !matches!(
        provider_type.trim().to_ascii_lowercase().as_str(),
...
pub(crate) fn is_codex_cli_backend_url(url: &str) -> bool {
    let url = url.trim().to_ascii_lowercase();
    url.contains("/codex") && (url.contains("/backend-api/") || url.contains("/backendapi/"))
}

pub(crate) fn provider_type_is_fixed_for_admin_oauth(provider_type: &str) -> bool {
    provider_type_is_fixed(provider_type)
}

pub(crate) fn provider_type_admin_oauth_template(
    provider_type: &str,
...
pub(crate) const ADMIN_PROVIDER_OAUTH_TEMPLATE_TYPES: &[&str] =
