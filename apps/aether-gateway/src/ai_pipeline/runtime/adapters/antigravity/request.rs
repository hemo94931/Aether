...
pub(crate) enum AntigravityEnvelopeRequestType {
    Agent,
    EndpointTest,
}
...
pub(crate) enum AntigravityRequestEnvelopeSupport {
    Supported(Value),
    Unsupported(AntigravityRequestEnvelopeUnsupportedReason),
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub(crate) enum AntigravityRequestEnvelopeUnsupportedReason {
    NonObjectBody,
    MissingContents,
    MissingRequestId,
    MissingModel,
...
pub(crate) fn classify_antigravity_safe_request_body(
    request_body: &Value,
...
pub(crate) fn build_antigravity_safe_v1internal_request(
