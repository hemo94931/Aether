...
fn map_ldap_user_auth_row(
    row: &sqlx::postgres::PgRow,
) -> Result<StoredLdapAuthUserRow, DataLayerError> {
    Ok(StoredLdapAuthUserRow {
...
async fn find_ldap_auth_user_for_update_in_tx(
    ...
    if let Some(ldap_dn) = ldap_dn.filter(|value| !value.trim().is_empty()) {
        ...
        if let Some(row) = row.as_ref() {
            return map_ldap_user_auth_row(row).map(Some);
        }
    }
    ...
    let row = sqlx::query(FIND_LDAP_AUTH_USER_BY_EMAIL_SQL)
        ...
        .fetch_optional(&mut **tx)
        .await?;
    row.as_ref().map(map_ldap_user_auth_row).transpose()
}
...
impl GatewayDataState {
    ...
    pub(crate) async fn get_or_create_ldap_auth_user(
        ...
        if let Some(existing) = existing {
            if existing.user.is_deleted || !existing.user.is_active {
                tx.commit().await?;
                return Ok(None);
            }
            if !existing.user.auth_source.eq_ignore_ascii_case("ldap") {
                tx.commit().await?;
                return Ok(None);
            }

            let email_changed = existing.user.email.as_deref() != Some(email.as_str());
            if email_changed {
    ...
    pub(in crate::gateway::gateway_data) async fn find_auth_api_key_snapshot(
        &self,
    ...
    pub(in crate::gateway) async fn list_auth_api_key_snapshots_by_ids(
        &self,
        api_key_ids: &[String],
    ...
    pub(crate) async fn read_auth_api_key_snapshot_by_key_hash(
        ...
        read_auth_api_key_snapshot_by_key_hash(self, key_hash, now_unix_secs).await
    }
}
