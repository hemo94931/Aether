...
impl GatewayDataState {
    ...
    #[allow(dead_code)]
    pub(in crate::gateway) fn encryption_key(&self) -> Option<&str> {
    ...
    pub(crate) async fn list_system_config_entries(
        ...
        match self
            ...
            .and_then(|backends| backends.postgres())
        {
            Some(backend) => backend.list_system_config_entries().await.map(|rows| {
                rows.into_iter()
                    .map(|(key, value, description, updated_at_unix_secs)| {
    ...
    pub(crate) async fn upsert_system_config_entry(
            ...
            .and_then(|backends| backends.postgres())
        {
            Some(backend) => backend
                ...
                .await
                .map(
                    |(key, value, description, updated_at_unix_secs)| StoredSystemConfigEntry {
                        key,
                        value,
                        description,
                        updated_at_unix_secs,
    ...
    pub(crate) async fn read_admin_system_stats(
        &self,
    ) -> Result<(u64, u64, u64, u64), DataLayerError> {
        match self
            .backends
            .as_ref()
            .and_then(|backends| backends.postgres())
