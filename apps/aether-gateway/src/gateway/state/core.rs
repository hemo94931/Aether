...
use crate::gateway::maintenance::spawn_wallet_daily_usage_aggregation_worker;

impl AppState {
    pub(in crate::gateway) fn replace_data_state(&mut self, data: Arc<GatewayDataState>) {
        self.clear_provider_transport_snapshot_cache();
        self.tunnel = crate::gateway::tunnel::EmbeddedTunnelState::with_data(Arc::clone(&data));
        self.data = data;
    }
    ...
    fn build(
        ...
        Ok(Self {
            ...
            tunnel: crate::gateway::tunnel::EmbeddedTunnelState::with_data(data),
    ...
    pub fn with_tunnel_identity(
        ...
        self.tunnel = crate::gateway::tunnel::EmbeddedTunnelState::with_data_and_identity(
            Arc::clone(&self.data),
            instance_id,
            relay_base_url,
            90,
    ...
    pub(crate) async fn list_system_config_entries(
        &self,
    ) -> Result<Vec<crate::gateway::gateway_data::StoredSystemConfigEntry>, GatewayError> {
        self.data
    ...
    pub(crate) async fn upsert_system_config_entry(
        ...
        value: &serde_json::Value,
        description: Option<&str>,
    ) -> Result<crate::gateway::gateway_data::StoredSystemConfigEntry, GatewayError> {
        self.data
    ...
    pub(crate) async fn read_admin_system_stats(
        &self,
    ) -> Result<(u64, u64, u64, u64), GatewayError> {
        self.data
    ...
    pub fn spawn_background_tasks(&self) -> Vec<JoinHandle<()>> {
        ...
        if let Some(handle) =
            crate::gateway::wallet_runtime::spawn_provider_quota_reset_worker(self.data.clone())
        {
            tasks.push(handle);
        }
