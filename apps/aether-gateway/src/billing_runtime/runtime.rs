...
use crate::gateway::usage::{UsageEvent, UsageEventType};

pub(crate) async fn enrich_usage_event_with_billing(
...
fn merge_billing_snapshot_metadata(
    ...
    snapshot: &aether_billing::BillingSnapshot,
    rate_multiplier: f64,
    is_free_tier: bool,
) -> Result<(), DataLayerError> {
...
mod tests {
    ...
    use aether_data::repository::billing::{
        InMemoryBillingReadRepository, StoredBillingModelContext,
    };
    use serde_json::json;
    use serde_json::Value;

    use super::enrich_usage_event_with_billing;
    use crate::gateway::gateway_data::GatewayDataState;
    use crate::gateway::usage::{UsageEvent, UsageEventData, UsageEventType};

    fn billing_state() -> GatewayDataState {
        let repository = Arc::new(InMemoryBillingReadRepository::seed(vec![
        ...
        GatewayDataState::with_billing_reader_for_tests(repository)
    }
    ...
    async fn enriches_completed_usage_event_with_billing_snapshot() {
        ...
        enrich_usage_event_with_billing(&billing_state(), &mut event)
            .await
