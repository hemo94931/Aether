...
pub(crate) fn header_rules_are_locally_supported(rules: Option<&Value>) -> bool {
...
pub(crate) fn body_rules_are_locally_supported(rules: Option<&Value>) -> bool {
    let Some(rules) = rules else {
        return true;
    };
    let Some(rules) = rules.as_array() else {
...
pub(crate) fn apply_local_body_rules(
    body: &mut Value,
    rules: Option<&Value>,
    original_body: Option<&Value>,
) -> bool {
