...
pub(crate) fn build_passthrough_headers(
    headers: &http::HeaderMap,
    extra_headers: &BTreeMap<String, String>,
...
pub(crate) fn build_openai_passthrough_headers(
    headers: &http::HeaderMap,
    auth_header: &str,
    auth_value: &str,
    extra_headers: &BTreeMap<String, String>,
...
pub(crate) fn ensure_upstream_auth_header(
...
pub(crate) fn resolve_local_openai_chat_auth(
...
pub(crate) fn resolve_local_standard_auth(
    transport: &GatewayProviderTransportSnapshot,
) -> Option<(String, String)> {
    let auth_type = transport.key.auth_type.trim().to_ascii_lowercase();
    let secret = transport.key.decrypted_api_key.trim();
