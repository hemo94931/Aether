...
use crate::gateway::ai_pipeline::runtime::provider_types::is_codex_cli_backend_url;
use url::form_urlencoded;

pub(crate) fn build_openai_chat_url(upstream_base_url: &str, query: Option<&str>) -> String {
...
pub(crate) fn build_openai_cli_url(
...
pub(crate) fn build_claude_messages_url(upstream_base_url: &str, query: Option<&str>) -> String {
    let (trimmed, base_query) = split_base_url_query(upstream_base_url);
    let trimmed = trimmed.trim_end_matches('/');
    let mut url = if trimmed.ends_with("/v1") {
...
pub(crate) fn build_gemini_video_predict_long_running_url(
    upstream_base_url: &str,
    model: &str,
    query: Option<&str>,
) -> Option<String> {
