...
pub(crate) struct LocalOAuthResolution {
    pub(crate) auth: Option<LocalResolvedOAuthRequestAuth>,
    pub(crate) refreshed_entry: Option<CachedOAuthEntry>,
    pub(crate) refresh_in_flight: bool,
}

#[derive(Debug, Clone, PartialEq)]
pub(crate) struct CachedOAuthEntry {
...
#[derive(Debug, Error)]
pub(crate) enum LocalOAuthRefreshError {
...
#[async_trait]
pub(crate) trait LocalOAuthRefreshAdapter: Send + Sync {
...
pub(crate) struct LocalOAuthRefreshCoordinator {
...
impl LocalOAuthRefreshCoordinator {
    ...
    pub(crate) async fn resolve_with_result(
        &self,
        client: &reqwest::Client,
        transport: &GatewayProviderTransportSnapshot,
    ...
    #[cfg(test)]
    pub(crate) fn with_adapters_for_tests(
        adapters: Vec<Arc<dyn LocalOAuthRefreshAdapter>>,
    ) -> Self {
        Self {
            adapters,
...
pub(crate) fn supports_local_oauth_request_auth_resolution(
    transport: &GatewayProviderTransportSnapshot,
) -> bool {
